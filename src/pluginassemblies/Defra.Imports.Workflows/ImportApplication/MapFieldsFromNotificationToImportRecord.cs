...
namespace Defra.Imports.Workflows.ImportApplication
{
    [CrmPluginRegistration(
      nameof(MapFieldsFromNotificationToImportRecord),
      "Maps fields from notification record to import record using a mapping web resource",
      "Maps fields from notification record to import record using a xml mapping web resource",
      "Defra.Imports.Workflows.ImportApplication",
      IsolationModeEnum.Sandbox)]
    public class MapFieldsFromNotificationToImportRecord : WorkflowActivity
    {
        [Input("Importer Notification Record to map fields from")]
        [RequiredArgument]
        [ReferenceTarget("defraimp_importernotification")]
        public InArgument<EntityReference> NotificationToMapFrom { get; set; }
        ...
        {
            EntityReference notificationToMapFromRef = NotificationToMapFrom.GetRequired(context, nameof(this.NotificationToMapFrom));
            ...
            MapFieldsFromOneRecordToAnotherBusinessLogic mapFieldsBusinessLogic = new MapFieldsFromOneRecordToAnotherBusinessLogic(repoFactory, notificationToMapFromRef, importRecordToMapToRef, fieldMappingResourceRef, shouldOverwriteExisting);
