...
namespace Defra.Imports.BusinessLogic.Utils
{
    public class MapFieldsFromOneRecordToAnotherBusinessLogic
    {
        ...
        public MapFieldsFromOneRecordToAnotherBusinessLogic(IRepositoryFactory repositoryFactory, EntityReference entityFromRef, EntityReference entityToRef, string mappingXml, bool shouldOverwriteExisting)
        {
            _repositoryFactory = repositoryFactory;
            _entityFromRef = entityFromRef;
            _entityToRef = entityToRef;
            _mappingXml = mappingXml;
            _shouldOverwriteExisting = shouldOverwriteExisting;
        }

        public void RunLogic()
        {
            if (_mappingWebResourceRef != null)
            {
                ...
                {
                    IXmlRetriever webResourceXmlRetriever = new WebResourceXmlRetriever(_repositoryFactory, _mappingWebResourceRef);
                    _mappingXml = webResourceXmlRetriever.GetXml();
