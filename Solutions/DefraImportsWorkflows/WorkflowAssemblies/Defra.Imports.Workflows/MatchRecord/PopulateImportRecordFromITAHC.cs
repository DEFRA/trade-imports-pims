...
namespace Defra.Imports.Workflows.MatchRecord
{
    ...
    nameof(PopulateImportRecordFromITAHC),
    "Maps fields from ITAHC to Import Record using a mapping web resource",
    "Maps fields from ITAHC to Import Record using a xml mapping web resource",
    "Defra.Imports.Workflows.MatchRecord",
    IsolationModeEnum.Sandbox)]
    public class PopulateImportRecordFromITAHC : WorkflowActivity
    {
        ...
        [Input("ITAHC Record to copy from")]
        [RequiredArgument]
        ...
        internal override void ExecuteWorkflowActivity(CodeActivityContext context, IWorkflowContext workflowContext, IOrganizationService orgSvc, ILogWriter logWriter)
        {
            ...
            EntityReference matchRecordRef = MatchRecord.GetRequired(context, nameof(this.ITAHCToMapFrom));
            ...
            MapFieldsFromOneRecordToAnotherBusinessLogic mapFieldsBusinessLogic = new MapFieldsFromOneRecordToAnotherBusinessLogic(repoFactory, itahcToMapFromRef, importRecordToMapToRef, itahcXml, shouldOverwriteExisting);
