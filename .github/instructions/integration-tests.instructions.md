...
- **Test setup / pre-conditions**: use or extend a scenario step builder in `Defra.Imports.Scenarios` (e.g. `CreateImportApplication`, `CreateConsignee`) instead of a Marktek record generator. If the scenario you need doesn't exist yet, add a new step builder there rather than a Marktek-based one in `Defra.Imports.IntegrationTests`.
