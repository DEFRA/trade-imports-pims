...
- Select and justify implementation patterns, preferring Dataverse configuration, Power Automate cloud flows, synchronous workflows, custom actions, custom workflow activities, and custom APIs for new work; where reusable server-side code is required, prefer custom workflow activities over plug-ins. Apply the platform guidance that plug-ins are reserved for altering managed action/trigger behaviour or messages not supported by configuration/flows.
...
- **Power Automate design considerations** — selecting between cloud flows, synchronous workflows, custom actions, and custom workflow activities for new work; preferring custom workflow activities over plug-ins where reusable server-side behaviour is needed; cloud flow vs. plug-in choice, trigger scoping, error handling, and connector licensing.
