...
3. **Dataverse-first design** — prefer Dataverse-native, declarative, low-code capabilities (workflows, actions, business process flows, cloud flows) over plug-ins, custom code, external services, custom databases, or additional integration platforms; introduce these only when the platform cannot reasonably satisfy the requirement. Cloud flows have no synchronous equivalent, so for synchronous, server-side logic apply the hierarchy table-scoped business rules → real-time workflows → actions/custom workflow activities → plug-ins, using custom workflow activities where reusable input/output behaviour is required and plug-ins only where the earlier options cannot satisfy the requirement (e.g. to augment a managed action). Decompose high-level, end-to-end processes into their constituent rules/processes/actions rather than a single monolithic flow or workflow, so cross-cutting behaviours (e.g. validation) apply consistently wherever the underlying data or action is used.
...
- Design favouring Dataverse-native, declarative, low-code capabilities over plug-ins/custom code/external services (Principle 3), applying reuse-first thinking before proposing new components (Principle 2). If custom server-side code is required, prefer custom workflow activities as these allow for inputs/outputs and reuse within workflows and actions; reserve plug-ins for altering managed action behaviour or handling messages that workflows and flows cannot support.```
