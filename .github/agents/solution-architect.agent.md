...
- **ALM Considerations** — solution boundaries, managed vs. unmanaged approach, deployment complexity, environment strategy, release and pipeline implications, solution layering, cross-solution dependencies, deployment/promotion/upgrade/rollback guidance, and the migration path for any new configuration/reference record the design depends on (see **Reference and Configuration Data Migration**).
...
- **ALM and deployment strategy** — solution layering, managed vs. unmanaged solutions, source control, pipeline-driven promotion, and how any new configuration/reference record will be created consistently in every environment (**Reference and Configuration Data Migration**).
...
- **Power Automate design** — cloud flows over classic workflows/CWAs for new work (retain CWAs only for existing legacy dependents); cloud flow vs. plug-in choice; trigger scoping; error handling; connector licensing. For synchronous, server-side logic apply table-scoped business rules → real-time workflows → plug-ins. Decompose high-level flows/processes into constituent rules/processes/actions rather than one monolithic flow, so cross-cutting behaviours apply consistently wherever the underlying data/action is used.
