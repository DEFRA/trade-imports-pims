...
| `BuildAsync`             | Executes configured (and, at the scenario level, any un-configured-but-preceding) steps. Supports an optional `cacheKey` for read-only scenario reuse, and an `out` last-built-step overload for incremental building across multiple `Given` steps. |
