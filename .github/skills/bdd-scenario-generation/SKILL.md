...
- **Preconditions belong in `Given`, not `When`.** If a scenario needs data to already be in a particular state (e.g. a record already overdue, already assigned, already in a given status), express that as a `Given` (or `Given`/`And`) step, even when a UI-driven step binding exists that could reach the same state — using it as a `When` step misrepresents setup as the action under test. Reserve `When` exclusively for the single action or event the scenario is actually verifying. Prefer an API/data-level `Given` step for this setup over a UI-driven one, since seeding state is not itself business behaviour worth exercising through the UI, and flag a new API-level `Given` binding as needed if no equivalent exists yet.
...
- **Use a Gherkin table (`DataTable`) for any step that takes a list or set of values, never a comma-separated or "and"-joined list embedded in the step text.** A step whose pattern hard-codes a fixed list of items (e.g. field names, column names, expected rows) as literal text can only ever match that exact set of values, so it can never be reused by a future scenario that needs a different or partial list — it forces a near-duplicate step definition instead. Express the list as a table argument (e.g. `Then I see the following columns:` followed by a table) so the step definition parameterises over the table's rows and remains reusable for any future scenario with a different set of values. This applies whether the list is inline in prose (`the list shows A, B and C`) or already implied by an existing table-based binding elsewhere in the suite — prefer the table form for consistency.
- **Prefer Gherkin tables with header row only when referencing horizontally aligned UI components.** For UI elements that are laid out horizontally in the application — such as form tabs across the top of a record, or column headers across a view — the values belong in a single header-only row with no data rows beneath it (e.g. `| Tab A | Tab B |`, not a separate `| Tab |` header followed by one tab per row).
...
- **Condense precondition steps that provide no additional context.** If a `Given` step can be inferred to have occurred from the step following it and adds no other context (e.g. `a submitter has submitted a request` immediately followed by `the request's work item is assigned to me`), condense the steps into a single step that still expresses the meaningful precondition (e.g. `a request's work item is assigned to me`). Only keep `Given` steps when they carry information that is not implicitly known from reading the following step(s) e.g. `a submitter has submitted an expedited request`. Use the indefinite article (`a`/`an`) the first time an entity is introduced and reserve the definite article (`the`) for steps that reference entities created earlier in the scenario.
...
Every scenario that involves a persona interacting with the application UI must begin with exactly one login step, chosen from the login step definitions in [LoginSteps.cs](../../../tests/Defra.Imports.Specs/StepDefinitions/LoginSteps.cs), e.g.:
...
4. **Omit the login step entirely for scenarios verified completely off-system.** Some behaviours are never observed through the application UI by any persona — for example, an email delivered to an inbox, or a background/scheduled process's side effect that only ever surfaces outside Dataverse. If every `Given`, `When` and `Then` in the scenario is genuinely verified at the API/data level (setup via the API, the action triggered via the API — e.g. a manual trigger on the underlying cloud flow — and the assertion checking for the created record via the API), do not add a login step for form's sake; there is no persona whose UI session is relevant to the behaviour being tested. Only include a login step when at least one step in the scenario represents an actual persona interacting with the application.
...
2. **Feature file per action** — within the entity folder, one `.feature` file per distinct user-facing action or capability associated with that table (e.g. `View contacts.feature`, `View a contact.feature`, `Create a contact.feature`, `Update a contact.feature`). A feature file represents a single action, not the whole entity — do not combine multiple actions (e.g. Create and Update) into one feature file. A `View` feature must stay scoped to structural, read-only assertions intrinsic to viewing the record (e.g. tabs, fields, view columns) — it must not become a catch-all for negative-path assertions that belong to other actions, such as whether a command is visible. Command-availability checks are negative testing for the corresponding action and belong in that action's own feature file (e.g. `Reject a work item.feature`), asserting the command is or isn't available under the relevant precondition.
...
5. **When authoring a new step for a list of values, parameterise it with a `DataTable` rather than baking the list into the step's capture text** — see the Scenario Standards rule on Gherkin tables. Check the index for an existing table-based step covering the same kind of list (e.g. columns, fields) before inventing a new one.
